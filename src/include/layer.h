...
// This is the overlay structure. It is used to create a seperate layer over the current windows.
...
struct LayFuncs {
    ...
    void (*lf_LayFree) (void*); // should only free any data kept in flayer->l_data (but not flayer->l_data itself)
...
typedef struct Layer {
    canvas_t *l_cvlist; /* list of canvases displaying layer */
    int l_width;
    int l_height;
...
__attribute__((format(printf, 2, 3))) void LMsg (int, const char* fmt, ...);
