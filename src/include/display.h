...
typedef struct statusposstr_s {
    int row, col;
} statusposstr_t;

typedef struct mouse_parse_s {
...
typedef struct display_s {
    event_t d_blankerev;                           
    event_t d_blockedev;                          // blocked timeout
    event_t d_hstatusev;                          // hstatus changed event
    event_t d_idleev;                             // screen blanker
    event_t d_mapev;                              // timeout event
    event_t d_readev;                             // userfd read event
    event_t d_statusev;                           // timeout event
    event_t d_writeev;                            // userfd write event
    Layout *d_layout;                           // layout we're using
    window_t* d_fore;                             // pointer to fore window
    window_t *d_other;                            // pointer to other window
    bool d_auto_nuke;                           // autonuke flag
    bool d_hstatus;                             // hardstatus used
    bool d_insert;                              // insert mode flag
    bool d_revvid;                              // reverse video
    canvas_t *d_cvlist;                         // the canvases of this display
    canvas_t *d_forecv;                         // current input focus
    canvas_t d_canvas;                          // our canvas slice
    ...
    display_s *d_next;                          // linked list
    display_status_t d_status;                  // is status displayed?
    hard_status_t d_has_hstatus;                // display has hardstatus line
    ...
    int d_mousetrack;                           // set when user wants to use mouse even when the window does not
    ...
    void (*d_processinput) (char*, size_t);     // main event loop ???
#ifdef ENABLE_UTMP                               
...
display_t *MakeDisplay (char *, char *, char *, int, pid_t, struct mode *);
int CalcCost (char *);
...
extern display_t *display, *displays;
