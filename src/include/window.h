...
// wants to write to window: user writes to window or stdout/stderr of pseudo are duplicated to window
#define W_WW(w) (\
    !((w)->w_pwin) || \
...
#define W_RP(w) (\
    (w)->w_pwin && \
    ((w)->w_pwin->p_fdpat & ((F_PFRONT << (F_PSHIFT * 2)) | (F_PFRONT << F_PSHIFT))) \
)
...
typedef struct new_window_s {
	...
	int	start_at; // where to start the search for the slot
	int	wlock; // default writelock setting
} new_window_t;

typedef struct pseudowin_s {
	Event p_readev;
	Event p_writeev;
	char p_cmd[MAXSTR];
...
Window* GetWindowByNumber(uint16_t nwin);
int DoStartLog (Window* win, char*, int);
int MakeWindow (struct NewWindow* win);
int ObtainAutoWritelock (Display* display, Window* win);
int OpenDevice(char**, int, int*, char**);
int ReleaseAutoWritelock (Display* display, Window* win);
int RemakeWindow (Window *);
int SwapWindows (int, int);
int winexec (char **);
void CloseDevice (Window* win);
void FreePseudowin (Window *);
void FreeWindow (Window *);
void ResetWindow (Window* win);
void WindowDied (Window* win, int, int);
void nwin_compose (struct NewWindow *, struct NewWindow *, struct NewWindow *);
void zmodem_abort(Window* win, Display* display);
...
extern new_window_t nwin_undef, nwin_default, nwin_options;
