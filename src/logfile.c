...
static log_t* logroot = NULL;

static void 
changed_logfile(log_t* l) {
...
//
// If possible, need_fd will be open afterwards and refer to the object
// originally reffered by fd. fd will be closed then
//
// Works just like:
//      ```
//      fcntl(fd, DUPFD, need_fd);
//      close(fd);
//      ```
// need_fd is returned on success, else -1 is returned
int 
logfile_move_fd(int fd, int need_fd) {
    int ret = -1;
...
static int 
logfile_reopen(char* name, int wantfd, log_t* l) {
    int got_fd;
...
// If the logfile has been removed, truncated, unlinked or the like, return
// nonzero. The l->st structure initialised by logfopen is updated on every
// call
static int 
stolen_logfile(log_t* l) {
    struct stat o, *s = l->st;
...
// FIXME: write and flush both *should* check the file's stat, if it disappeared
// or changed, re-open it.
int 
logfwrite(log_t* l, char* buf, size_t n) {
    int r;
