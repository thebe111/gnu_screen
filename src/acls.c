...
#include <syslog.h>
#include "include/screen.h"     /* includes acls.h */
...
static int AclSetPermWin(struct acluser*, struct acluser*, char*, window_t* win);
static int GrowBitfield(AclBits*, int, int, int);
static int UserAcl(struct acluser*, struct acluser**, int, char**);
static int UserAclCopy(struct acluser** to, struct acluser** from);
